...
public class Q1Mapper extends Mapper<LongWritable, Text, Text, FloatWritable>{
	...
	{	
		...
		//indicator that represents females that graduated from post secondary education
		if (rowStr[index].equals("SE.SEC.CUAT.PO.FE.ZS")) 
		{
			for (int x = index; x < rowStr.length; x++)
			{
				try
				{
					...
					context.write(new Text(rowStr[2]) , new FloatWritable(value));	
				}
				catch(Exception nfe)
				{
					continue;
			...
			//context.write(new Text(rowStr[1]), new FloatWritable(Float.parseFloat(rowStr[index2000Year])));
		}
